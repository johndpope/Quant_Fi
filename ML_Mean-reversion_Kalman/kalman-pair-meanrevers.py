...
def initialize(context):
    """
    This block is called once at the start of the algorithm.
    """
    # Quantopian specific varialbes
    set_slippage(slippage.VolumeShareSlippage(volume_limit=0.025, price_impact=0.1))
    set_commission(commission.PerShare(cost=0.0075, min_trade_cost=0.0))

    context.asset_pairs = [[symbol('HBAN'), symbol('EWBC'), {'in_short': False, 'in_long': False, 'spread': np.array([]), 'hedge_history': np.array([])}],
                          [symbol('FITB'), symbol('ZION'), {'in_short': False, 'in_long': False, 'spread': np.array([]), 'hedge_history': np.array([])}],
                          [symbol('WAFD'), symbol('OZRK'), {'in_short': False, 'in_long': False, 'spread': np.array([]), 'hedge_history': np.array([])}],
                          [symbol('DOX'), symbol('ACGL'), {'in_short': False, 'in_long': False, 'spread': np.array([]), 'hedge_history': np.array([])}] 
                           ]
    '''
    Specify how many days the market was open
    before we start refering to the Z-score of a pair
    '''
    context.z_back = 20
    context.hedge_lag = 2
    '''
    Specify the min abs value that the Z-score needs to be
    before considering opening a position on the pair.
    '''
    context.entry_z = 1.5
    '''
    If the Z-score goes above this rate we consider the spread too wide
    and cut our loss by closing any open position
    '''
    context.stoploss_z = 4.5
    
    # Specify at what times you want my_handle_data to run.
...
# Follwing Classes are used to test the pairs we trade on
class ADF(object):
    """
...
def process_pair(pair, context, data):
    ...
    # Close order logic
    if in_short and z_score > context.stoploss_z:
        print('z_score above 4. Exiting Trade')
        order_target(stock_1, 0)
        order_target(stock_2, 0)
        in_short = False
        in_long = False
        return [stock_1, stock_2, {'in_short': in_short, 'in_long': in_long, 'spread': spread, 'hedge_history': hedge_history}]
    elif in_short and z_score < 0.0:
        order_target(stock_1, 0)
        order_target(stock_2, 0)
        in_short = False
        in_long = False
